...
There is no *production* concrete agent in this repo — most design decisions here are validated empirically against how `moodle-agent` actually used them — several source comments say "confirmed empirically" and reference specific incidents from that consuming repo. Trust those comments; they encode real SDK behavior that isn't always documented, or fixes for concrete bugs seen in production use. The example agents (`examples/captain-whiskers/` here, and the sibling `student-agent`) are demonstration only, not a source of their own "confirmed empirically" findings:
...
- `student-agent` (sibling repo `../student-agent`, no longer under `examples/`) is the real-size counterpart: a full port of `moodle-agent`'s `student` role (20 skills, 7 slash commands, 3 opt-in subagents, an external Playwright MCP server, its own workspace-directory config) that consumes this kit through a `file:` dependency. `moodle-agent` itself is standalone (doesn't use this kit) — student-agent is a from-scratch reconstruction on top of it, and the place where gaps in the kit get discovered (e.g. `runChatTui()`'s `initialPrompt`).
...
With `config.contextDir` set, the session gets `Read`, `Write`, `Edit`, `Glob` and `Grep`. `cwd` is `projectDir`, so the SDK's own working-directory scope is the whole project — including whatever else lives there (a config file with a password, the user's own `context/`). `createFileScopeGate()` (`src/core/hooks/fileScopeGate.ts`, a `PreToolUse` hook registered alongside the transcript logger, so it also covers subagents) is the real boundary: `Write`/`Edit` only inside `knowledgeDir`, `sourcesDir` and `extraWritableDirs`; `Grep` only inside those plus `contextDir` (it prints file contents, so the project root is out, and so is any folder containing a denied path); `Read` anywhere except `deniedPaths`, which are also never writable or searchable. `Glob` only lists names and is left alone. Bash (subagents only) is not covered — which is why Bash-granting features stay opt-in in consumers. The pure decision is `checkFileScope()`, unit-tested in `test/core/hooks/fileScopeGate.test.ts`.

`sourcesDir` (optional) separates original files from the notes the agent writes: when set, the file-copy MCP tool is `save_to_sources` (copies into it) instead of `save_to_knowledge`. Both come from `createSaveFileServer()` in `src/core/tools/saveToKnowledge.ts` and share the `knowledgeFiles` server name, so `createFriendlyToolLabel()` unwraps either.

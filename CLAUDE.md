...
### Built-in knowledge vault

The kit ships the *behavior* for the notes folder, not just the permissions: whenever `config.knowledgeDir` is set and `spec.vault !== false`, `buildSessionOptions()` appends `vaultPromptSection()` (`src/core/vault.ts`: layers, layout, working rules — the schema of Karpathy's LLM-wiki pattern) to the spec's system prompt and adds `vaultPluginRoot()` (`assets/vault-plugin/`, shipped via `files` in `package.json`, resolved relative to the compiled module so it works from `dist/` and under tsx) to the plugin roots. The plugin (name `vault`) has the skills `vault-pages` (page templates), `vault-ingest`, `vault-query`, `vault-lint` and the commands `/vault:ingest`, `/vault:query`, `/vault:lint`. Everything in it is domain-agnostic (summaries/concepts/entities/syntheses); an agent with its own page types sets `vault: false` and writes its own rules. Skills in the plugin are in English like the repo's own skills.

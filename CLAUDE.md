...
It was extracted from a concrete agent built on it, so the generic parts could be versioned and reused independently. It has its own repo and its own versioning (currently 0.x — API not yet stable), is **not yet published to npm**, and is consumed by sibling agent repos via a `file:` dependency pointing here (see `install-links` note in git history for how that consumption works across a Windows/WSL boundary).

There is no *production* concrete agent in this repo — most design decisions here are validated empirically against how a real consuming agent used them — several source comments say "confirmed empirically" and reference specific incidents from it. Trust those comments; they encode real SDK behavior that isn't always documented, or fixes for concrete bugs seen in production use. The example agents (`examples/captain-whiskers/` here, and the sibling `student-agent`) are demonstration only, not a source of their own "confirmed empirically" findings:

- `examples/captain-whiskers/` is a toy, and a fully standalone project (its own `package.json`/lockfile, depends on the kit via `file:../..` and imports only from the package root — so it needs `npm run build` here first, and root `eslint` ignores `examples/`): a minimal joke-telling agent that exercises the kit end-to-end (`AgentSpec`, `buildSessionOptions()`, `runChatTui()`, `ensureClaudeAuth()`, a plugin-provided skill and slash command via `pluginRoots()`). It always runs in `mode: "autonomous"` (hardcoded, no CLI argument to change it), so it doesn't demonstrate the approval checkpoint (`request_human_approval`, `guided`/`interactive` modes) — see `tools/humanApproval.ts` and `hooks/stepGate.ts` for that behavior instead.
- `student-agent` (sibling repo `../student-agent`, no longer under `examples/`) is the real-size counterpart: an agent that takes a Moodle course as a student (20 skills, 7 slash commands, 3 opt-in subagents, an external Playwright MCP server, its own workspace-directory config) that consumes this kit as a dependency, and the place where gaps in the kit get discovered (e.g. `runChatTui()`'s `initialPrompt`).
...
- `subagentTypeGate.ts` — restricts the `Agent` tool's `subagent_type` to whatever `buildSubagents()` actually registered. Without this, the SDK's own built-in `"general-purpose"` subagent type is spawnable regardless of what's registered, inherits the *whole* session's tools (Bash included), and was used in practice (in a real consuming agent) to route around the Bash gate — the model delegated a plain file deletion to a spontaneous general-purpose agent just to get at Bash.
...
### File tools and their scope

With `config.knowledgeDir` and/or `config.sourcesDir` set, the session gets `Read`, `Write`, `Edit`, `Glob` and `Grep` (neither set: no file tools at all — there is no separate "context" folder; material the user provides is just more of `sourcesDir`). `cwd` is `projectDir`, so the SDK's own working-directory scope is the whole project — including whatever else lives there (a config file with a password). `createFileScopeGate()` (`src/core/hooks/fileScopeGate.ts`, a `PreToolUse` hook registered alongside the transcript logger, so it also covers subagents) is the real boundary: `Write`/`Edit` only inside `knowledgeDir` and `extraWritableDirs`; `Grep` only inside those plus `sourcesDir` (it prints file contents, so the project root is out, and so is any folder containing a denied path); `Read` anywhere except `deniedPaths`, which are also never writable or searchable. `Glob` only lists names and is left alone. Bash (subagents only) is not covered — which is why Bash-granting features stay opt-in in consumers. The pure decision is `checkFileScope()`, unit-tested in `test/core/hooks/fileScopeGate.test.ts`.

`sourcesDir` holds the originals as obtained (raw layer) apart from the agent's notes (`knowledgeDir`, the wiki layer): it is readable and searchable but deliberately **not** writable through `Write`/`Edit`. The only way the agent adds to it is the `save_to_sources` MCP tool (`src/core/tools/saveToSources.ts`, server name `sourceFiles`, registered whenever `sourcesDir` is set), which copies from the run directory with `COPYFILE_EXCL` so an existing original is never overwritten; `createFriendlyToolLabel()` unwraps its server name.

### Built-in knowledge base

The kit ships the *behavior* for the notes folder, not just the permissions: whenever `config.knowledgeDir` is set and `spec.knowledgeBase !== false`, `buildSessionOptions()` appends `knowledgePromptSection()` (`src/core/knowledge.ts`: layers, layout, working rules — the schema of Karpathy's LLM-wiki pattern) to the spec's system prompt and adds `knowledgePluginRoot()` (`assets/knowledge-plugin/`, shipped via `files` in `package.json`, resolved relative to the compiled module so it works from `dist/` and under tsx) to the plugin roots. The plugin (name `knowledge`) has the skills `knowledge-pages` (page templates), `knowledge-ingest`, `knowledge-query`, `knowledge-lint` and the commands `/knowledge:ingest`, `/knowledge:query`, `/knowledge:lint`. Everything in it is domain-agnostic (summaries/concepts/entities/syntheses); an agent with its own page types sets `knowledgeBase: false` and writes its own rules. Skills in the plugin are in English like the repo's own skills.

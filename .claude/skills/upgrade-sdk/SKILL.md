...
- `skills: "all"` enables the `Skill` tool without listing it in `allowedTools`; plugin skills are discovered from `plugins: [{ type: "local", ... }]` with a `.claude-plugin/plugin.json` manifest, and plugin commands are namespaced `/<plugin>:<command>`.

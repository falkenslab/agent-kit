...
{
  "dependencies": {
    "@falkenslab/agent-kit": "git+https://github.com/falkenslab/agent-kit.git#v0.6.0"
  }
}
```

- Sin `#<tag>` instala la rama por defecto (`main`); mejor fijar siempre un tag, o un rango con `#semver:^0.6.0`.
- Si tu configuración de npm restringe los scripts de instalación, permite el de este paquete con `"allowScripts": { "@falkenslab/agent-kit@0.6.0": true }` en tu `package.json`.

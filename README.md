...
{
  "dependencies": {
    "@falkenslab/agent-kit": "git+https://github.com/falkenslab/agent-kit.git#v0.2.0"
  }
}
```

- Sin `#<tag>` instala la rama por defecto (`main`); mejor fijar siempre un tag, o un rango con `#semver:^0.2.0`.
- Si tu configuración de npm restringe los scripts de instalación, permite el de este paquete con `"allowScripts": { "@falkenslab/agent-kit@0.2.0": true }` en tu `package.json`.
...
- **Herramientas de fichero con ámbito** (`core/hooks/fileScopeGate.ts`) — con `contextDir` configurado, el agente tiene `Read`, `Write`, `Edit`, `Glob` y `Grep`. Un hook `PreToolUse` limita `Write`/`Edit` a `knowledgeDir`, `sourcesDir` y `extraWritableDirs`, limita `Grep` a esas carpetas más `contextDir`, y bloquea leer, buscar o escribir en `deniedPaths` (p. ej. un fichero de configuración con una contraseña). Sin él, la única frontera sería el directorio de trabajo entero del proyecto.
- **Herramientas `save_to_knowledge` / `save_to_sources`** (`core/tools/saveToKnowledge.ts`) — copian un archivo generado durante la ejecución (ej. algo recién descargado) a `knowledge/`, o a `sources/` si configuras `sourcesDir` para guardar los originales aparte de las notas, para que quede legible en sesiones futuras.

...
/**
 * The built-in knowledge vault (an "LLM wiki"): `sourcesDir` holds the originals, `knowledgeDir`
 * is the wiki the agent writes and maintains, and the rules below plus the plugin's skills
 * (vault-pages, vault-ingest, vault-query, vault-lint) and commands (/vault:ingest, /vault:query,
 * /vault:lint) are the schema that tells the agent how. Domain-agnostic on purpose: an agent
 * with its own page types (topics, activities...) opts out with `AgentSpec.vault: false` and
 * writes its own rules, or extends these.
 */

/** Absolute path of the plugin shipped with the kit — `assets/vault-plugin` next to `dist/` (or `src/` under tsx). */
export function vaultPluginRoot(): string {
  return path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..", "..", "assets", "vault-plugin");
}
...
/**
 * The "Knowledge vault" section appended to the system prompt: the layout and the working
 * rules. The exact page templates live in the `vault-pages` skill, to keep this short.
 */
export function vaultPromptSection(projectDir: string, knowledgeDir: string, sourcesDir?: string): string {
  const notes = shown(projectDir, knowledgeDir);
  const originals = sourcesDir ? shown(projectDir, sourcesDir) : undefined;
  return `## Knowledge vault (${notes})
\`${notes}\` is your memory across sessions, kept as an interlinked vault of markdown pages that you write and maintain yourself — a wiki, not a pile of notes. A future session only knows what is written there, so anything worth remembering must end up in a page, not just in this turn's reply. Write page content in the language you are using with the human; file names stay lowercase ASCII with hyphens.

### Layers
${originals ? `- **Originals (read-only for you)**: \`${originals}\`. Never rewrite one; build pages *about* them. Add a file there only with \`save_to_sources\`.\n` : ""}- **The vault**: \`${notes}\`, entirely yours to write.
- **The schema**: these rules plus the \`vault-pages\` skill (exact template of every page type — load it before creating a page) and the \`vault-ingest\`, \`vault-query\` and \`vault-lint\` skills.

### Layout
\`\`\`
${notes}
  index.md          catalog of every page, one line each: read it FIRST, always
  log.md            append-only log of what you did to the vault
  overview.md       living synthesis of the whole vault
...
### Working rules
- Start by reading \`index.md\`, then only the pages the task needs — don't read the whole vault.
- Links are standard markdown links relative to the page, and go both ways: when A links to B, add the link back in B. To find who links to a page, \`Grep\` for its file name inside the vault.
...
- Every claim must be traceable to a summary page, an original, or — clearly labelled as outside the vault — the web.
